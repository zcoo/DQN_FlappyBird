...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    while 1:
        ...
        if t > OBSERVE/100:
            ...
            # TODO
            s_j_batch=[abatch[0] for abatch in minibatch]
            a_batch=[abatch[1] for abatch in minibatch]
            reward_batch=[abatch[2] for abatch in minibatch]
            s_j1_batch=[abatch[3] for abatch in minibatch]
            #更新公式
            readout_j1 = readout.eval(feed_dict={s: s_j1_batch})
            y_batch = []
            for i in range(0, BATCH):
                ifTerminal = minibatch[i][4]
                if ifTerminal == True:
                    y_batch.append(reward_batch[i])
                else:
                    y_batch.append(reward_batch[i] + GAMMA * np.max(readout_j1[i]))
